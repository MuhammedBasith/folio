...
async function nextReference(ownerId: string): Promise<string> {
  ...
  const current = latest
    ? Number.parseInt(latest.reference.replace(REFERENCE_PREFIX, ""), 10)
    : 0;
...
export async function updateOrder(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (!isOrderEditable(existing._count.payments)) {
      throw orderLocked();
    }
 ...
 * Deletes an order.
 *
 * Also refused once payments exist. Removing an order that has money recorded
 * against it destroys the record of that money, which is exactly the thing this
